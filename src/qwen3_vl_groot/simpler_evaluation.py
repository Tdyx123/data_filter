...
def bridge_actions_to_simpler(value: Any) -> np.ndarray:
    ...
    result[..., 6] = np.where(actions[..., 6] > 0.5, 1.0, -1.0)
    return result
...
def _step_environment(environment: Any, action: np.ndarray) -> tuple[Any, bool, bool, dict[str, Any]]:
    try:
...
def run_simpler_episode(
    *,
    ...
    if denoising_steps <= 0 or max_steps <= 0:
        raise SimplerEvaluationError("denoising_steps and max_steps must be positive")
    try:
    ...
    while steps < max_steps and not success and not truncated:
        ...
        model_image = preprocess_bridge_image(
            source_image,
            resize_size=256,
            crop_size=crop_size,
        ...
        predicted = policy.predict_actions(
            model_image,
            proprio,
            task.instruction,
            denoising_steps=denoising_steps,
        ...
        if actions.shape != (1, 8, 7):
            raise SimplerEvaluationError(
                f"Qwen checkpoint returned actions with shape {actions.shape}; expected (1, 8, 7)"
            )
        simpler_actions = bridge_actions_to_simpler(actions[0])
...
def _summary(episodes: Sequence[Mapping[str, Any]], tasks: Sequence[SimplerTaskSpec]) -> dict[str, Any]:
...
def _check_output_targets(settings: SimplerEvaluationSettings) -> None:
    targets = [
...
def validate_simpler_source(
    ...
    *,
    ...
    if missing_assets:
        raise SimplerEvaluationError(f"SimplerEnv visual-matching asset is missing: {missing_assets}")
    return {
    ...
    *,
    ...
    if builder is None:
        try:
            ...
            raise SimplerEvaluationError(
                "Could not import SimplerEnv; run through scripts/evaluate_simpler_qwen.sh"
            ) from error
        ...
        try:
            ...
            if inference is None:
                ...
                if predicted_actions.shape != (1, 8, 7):
                    raise SimplerEvaluationError(
                        "Qwen preflight returned actions with shape "
                        f"{predicted_actions.shape}; expected (1, 8, 7)"
                    )
                bridge_actions_to_simpler(predicted_actions)
                inference = {
                    "task": task.key,
            ...
            environments.append(
                {
                    ...
                    "model_image_shape": list(model_image.shape),
