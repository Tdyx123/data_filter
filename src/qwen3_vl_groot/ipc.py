"""Authenticated local Unix IPC for split Qwen/SimplerEnv evaluation."""
...
class QwenIPCError(RuntimeError):
    """Raised when the local Qwen policy service rejects or loses a request."""
...
def _decode_array_payload(
    ...
    *,
    ...
    if not isinstance(payload, Mapping):
        raise QwenIPCError(f"{name} payload must be a mapping")
    ...
    if type(shape) is not list or any(
        ...
        raise QwenIPCError(f"{name} shape must contain positive integers")
...
def _validate_instruction(instruction: Any) -> str:
    if not isinstance(instruction, str) or not instruction.strip():
        raise QwenIPCError("infer instruction must be non-empty")
...
class _PolicySession:
    ...
    def route(self, request: Any) -> tuple[dict[str, Any], bool]:
        if not isinstance(request, Mapping):
            raise QwenIPCError("IPC request must be a mapping")
        ...
        if request_type == "infer":
            if self.generator is None:
                raise QwenIPCError("reset_rng must be called before infer")
            ...
            if not np.all(np.isfinite(actions)):
                raise QwenIPCError("policy returned NaN or infinite actions")
            return {
                "ok": True,
        ...
        raise QwenIPCError(f"Unsupported IPC request type: {request_type!r}")
...
def serve_policy(
    *,
    ...
    if not authkey:
        raise QwenIPCError("IPC authkey must be non-empty")
    target.parent.mkdir(parents=True, exist_ok=True)
    if target.exists():
        raise QwenIPCError(f"IPC socket already exists: {target}")
...
class QwenIPCClient:
    ...
    def _request(self, payload: Mapping[str, Any]) -> Any:
        if self._closed:
            raise QwenIPCError("Policy client is closed")
        try:
            self._connection.send(dict(payload))
            response = self._connection.recv()
        except (EOFError, OSError) as error:
            raise QwenIPCError(f"Policy server connection failed: {error}") from error
        if not isinstance(response, Mapping):
            raise QwenIPCError("Policy server returned a non-mapping response")
        if not response.get("ok"):
            raise QwenIPCError(str(response.get("error", "Unknown policy server error")))
        return response.get("data")

    def metadata(self) -> dict[str, Any]:
        data = self._request({"type": "metadata"})
        if not isinstance(data, Mapping):
            raise QwenIPCError("Policy metadata must be a mapping")
        return dict(data)
    ...
    def reset_rng(self, seed: int) -> int:
        ...
        if not isinstance(data, Mapping) or "seed" not in data:
            raise QwenIPCError("Policy reset response is missing the seed")
    ...
    def infer(
        ...
        if not isinstance(data, Mapping) or "actions" not in data:
            raise QwenIPCError("Policy response is missing the actions payload")
        ...
        if not np.all(np.isfinite(actions)):
            raise QwenIPCError("policy server returned NaN or infinite actions")
        return actions
