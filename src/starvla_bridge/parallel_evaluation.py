"""Multi-replica orchestration for StarVLA SimplerEnv evaluation."""
...
from .ipc import StarVLAIPCClient
...
def _evaluation_arguments(arguments: argparse.Namespace) -> argparse.Namespace:
    ...
    return build_evaluation_parser().parse_args(
        [
            "--socket",
            "/tmp/starvla-parallel-planning.sock",
            "--auth-key-hex",
            "00",
            "--output-dir",
...
def _write_failure(
    ...
    *,
    ...
    report = {
        "schema_version": 1,
        "status": "failed",
        "route": "qwen3vl-groot-starvla-simpler-widowx-parallel-eval",
    ...
    try:
        ...
        for replica in simulator_replicas:
            ...
            command = [
                ...
                "-m",
                "starvla_bridge.evaluate_simpler",
                "--socket",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"StarVLA parallel evaluation error: {error}", file=sys.stderr)
        return error.exit_code
    except Exception as error:
        print(
            f"StarVLA parallel evaluation failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
        return 1
    finally:
