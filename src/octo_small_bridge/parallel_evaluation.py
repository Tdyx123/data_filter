"""Multi-replica orchestration for Octo-small SimplerEnv evaluation."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run sharded Octo-small SimplerEnv evaluation across model replicas."
    )
...
def _evaluation_arguments(arguments: argparse.Namespace) -> argparse.Namespace:
    ...
    return build_evaluation_parser().parse_args(
        [
            "--socket",
            "/tmp/octo-parallel-planning.sock",
            "--auth-key-hex",
            "00",
            "--output-dir",
...
def _validate_replica_metadata(replicas: Sequence[_Replica]) -> None:
    ...
    for replica in replicas:
        client = OctoIPCClient(replica.socket_path, authkey=replica.authkey)
        try:
...
def _write_failure(
    ...
    *,
    ...
    report = {
        "schema_version": 1,
        "status": "failed",
        "route": "octo-small-bridge-simpler-widowx-parallel-eval",
...
def run_parallel(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    ipc_root = Path(tempfile.mkdtemp(prefix="octo-simpler-parallel."))
    ...
    try:
        for index, device in enumerate(active_devices):
            ...
            if arguments.statistics is not None:
                command.extend(("--statistics", str(arguments.statistics)))
        ...
        for replica in simulator_replicas:
            ...
            command = [
                ...
                "-m",
                "octo_small_bridge.evaluate_simpler",
                "--socket",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Octo parallel evaluation error: {error}", file=sys.stderr)
        return error.exit_code
    except Exception as error:
        print(
            f"Octo parallel evaluation failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
        return 1
    finally:
