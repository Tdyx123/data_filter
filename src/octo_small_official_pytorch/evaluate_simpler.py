...
from .ipc import OfficialIPCClient, OfficialIPCError
from .remote_policy import ACTION_POSTPROCESSING_MODES, OctoOfficialRemotePolicy


EVALUATION_ROUTE = "octo-small-official-pytorch-simpler-widowx-eval"
PREFLIGHT_ROUTE = "octo-small-official-pytorch-simpler-widowx-preflight"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate the remote official-semantics Octo-small PyTorch baseline."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/octo_small_official_pytorch_simpler_eval"),
    )
    ...
    parser.add_argument(
        ...
        help="Official horizon-4 temporal ensemble or first_action ablation.",
    )
    ...
    parser.add_argument(
        "--smoke-test",
        action="store_true",
        help="Run one seed/object episode per task for at most eight steps.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    client: OfficialIPCClient | None = None
    ...
    try:
        ...
        client = OfficialIPCClient(arguments.socket, authkey=authkey)
        policy = OctoOfficialRemotePolicy(
            client, action_postprocessing=arguments.action_postprocessing
        )
        ...
        return 3
    except (SimplerEvaluationError, OfficialIPCError) as error:
        _write_failure(arguments.output_dir, error=error, exit_code=2)
        print(f"Official Octo evaluation error: {error}", file=sys.stderr)
        return 2
    except Exception as error:
        _write_failure(arguments.output_dir, error=error, exit_code=1)
        print(
            f"Official Octo execution failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
        return 1
    finally:
