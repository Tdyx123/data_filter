...
DEFAULT_OUTPUT_ROOT="outputs/qwen_libero_eval"
SIMULATION_INFRASTRUCTURE_EXIT_CODE=3

usage() {
  cat <<'EOF'
Usage:
  bash scripts/evaluate_libero_qwen.sh --checkpoint STEP_DIR [--indexes LIST|all] [EVALUATOR_ARGS...]
  bash scripts/evaluate_libero_qwen.sh --checkpoint STEP_DIR --task-name NAME [EVALUATOR_ARGS...]

Launcher options:
  --checkpoint PATH  Required concrete step-XXXXXXXX Qwen LoRA checkpoint directory.
...
All remaining arguments are forwarded unchanged to:
  python3 -m qwen3_vl_groot.evaluate

Use that module's --help for evaluator options such as --model-path, --episodes,
--num-envs, --max-steps, --device, --denoising-steps, --policy-batch-size,
--preflight-only, and --smoke-test.
...
while (($# > 0)); do
  case "$1" in
    --checkpoint)
      [[ "${checkpoint_explicit}" == false ]] || fail_usage "--checkpoint may only be specified once"
      (($# >= 2)) || fail_usage "--checkpoint requires a value"
      [[ -n "$2" ]] || fail_usage "--checkpoint requires a non-empty value"
    ...
    --checkpoint=*)
      ...
      [[ -n "${checkpoint}" ]] || fail_usage "--checkpoint requires a non-empty value"
      shift
      ;;
    --indexes)
...
[[ "${checkpoint_explicit}" == true ]] || fail_usage "--checkpoint is required"
...
if [[ "${task_name_explicit}" == true ]]; then
  ...
  exec python3 -m qwen3_vl_groot.evaluate "${single_task_args[@]}"
fi
...
for index in "${selected_indexes[@]}"; do
  ...
  printf '[info] evaluating Qwen LoRA on LIBERO-10 task %d: %s\n' "${index}" "${task_name}"
  ...
  if python3 -m qwen3_vl_groot.evaluate "${task_args[@]}"; then
    printf '[info] task %d completed successfully\n' "${index}"
  else
    status=$?
    if ((status == 2)); then
      printf '[error] task %d hit an evaluation contract failure; aborting the batch\n' \
        "${index}" >&2
      exit 2
    fi
