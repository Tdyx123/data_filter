...
model_devices=""
sim_device="cuda:0"
output_dir="${project_root}/outputs/qwenvl_oft_simpler_eval"
server_timeout=600
forwarded=()
...
usage() {
  cat <<'EOF'
Usage: scripts/evaluate_simpler_qwenvl_oft.sh [launcher options] [evaluation options]
...
Launcher options:
  ...
  --checkpoint PATH      Qwen-VL OFT step-XXXXXXXX directory (required)
...
Evaluation options are forwarded to qwen_vl_oft.evaluate_simpler, including:
  --tasks TASKS --action-horizon 1 --preflight-only --smoke-test
  --save-videos-path PATH --video-fps FPS --overwrite

Qwen-VL OFT inference is deterministic and does not support denoising steps.
...
while (($#)); do
  case "$1" in
    ...
    --denoising-steps|--denoising-steps=*)
      fail "Qwen-VL OFT does not support --denoising-steps"
      ;;
    --pyenv-bin|--sim-python|--checkpoint|--model-path|--device|--model-devices|--sim-device|--output-dir|--server-timeout)
      option="$1"
      set_once "${option}"
...
if [[ -n "${model_devices}" ]]; then
  parallel_command=(
    "${pyenv_bin}" exec python -m qwen_vl_oft.parallel_evaluation
...
ipc_dir="$(mktemp -d "${TMPDIR:-/tmp}/qwen-oft-simpler.XXXXXXXX")"
chmod 700 "${ipc_dir}"
...
model_command=(
  "${pyenv_bin}" exec python -m qwen_vl_oft.server
  --socket "${socket_path}"
...
(
  export PYTHONPATH="${project_root}/src:${simpler_root}:${maniskill_root}"
  export MS2_REAL2SIM_ASSET_DIR="${maniskill_root}/data"
  exec "${sim_python}" -m qwen_vl_oft.evaluate_simpler \
    --socket "${socket_path}" \
    --auth-key-hex "${auth_key_hex}" \
    --output-dir "${output_dir}" \
    --sim-device "${sim_device}" \
